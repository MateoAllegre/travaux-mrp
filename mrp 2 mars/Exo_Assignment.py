...
def find_best_assignment(n, ac, cc):
    min_nsc = n*n
    nbExp = 10000
    for i in range(nbExp):
        aa, ca = generate_random_assignment(ac, cc)
        nsc = number_of_non_stable_couples(aa, ca, ac, cc)
        if nsc < min_nsc:
            min_nsc = nsc
            best_assignment = (aa, ca)
        if min_nsc == 0:
...
#------------------------------------------------------------------------------
def print_best_affectation(min, aa, agencies):
    print(min)
    for a in agencies:
        print(a + "  " + aa[a])
...
print_best_affectation(min, a[0], ["A1", "A2", "A3", "A4"])