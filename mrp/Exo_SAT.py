...
def set_of_variables_from_formula(f):
    ...
    for i in f:
        for j in i:
            formula_set.add(variable_of_literal(j))
    return formula_set
...
def boolean_value_of_clause(assignment, clause):
    """Given an assignment and a clause, returns the Boolean value of the
       clause."""
    for i in clause:
        if(boolean_value_of_literal(assignment, i)):
            return True
    return False
...
def boolean_value_of_formula(assignment, formula):
    """Given an assignment and a formula, returns the Boolean value of the
       formula."""
    for i in formula:
        if(boolean_value_of_clause(assignment, i) == False):
            return False
    return True
...
def number_of_true_clauses(assignment, formula):
    ...
    true_clause = 0
    for i in formula:
        if(boolean_value_of_clause(assignment, i)):
            true_clause += 1
    return true_clause
...
def pretty_print_formula(formula):
    ...
    vars = set_of_variables_from_formula(formula)
    biggest_size = max([len(x) for x in vars]) + 1
...
def pretty_print_assigned_formula(assignment, formula):
    ...
    for clause in formula:
        ...
        for lit in clause:
            ...
            to_print += " " * nb_spaces
            to_print += " or "
        print(to_print[:-4] + " = " + str(boolean_value_of_clause(assignment, clause)))
...
def iter_all_assignments(d):
    ...
    for x in vars:
        d[x] = False
    list_truth = [False] * nb_vars
    yield d
    
    while list_truth != [True] * nb_vars:
        for i in range(0, len(list_truth)):
            if list_truth[i]:
                list_truth[i] = False
                d[vars[i]] = list_truth[i]
            else:
                list_truth[i] = True
                d[vars[i]] = list_truth[i]
                break
...
def evaluate_all_assignments(formula):
    ...
    liter_dico = construct_dictionary_from_vars(set_of_variables_from_formula(formula))
